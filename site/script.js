...
(function(){
  ...
  // Message état vide (injecté une seule fois)
  let emptyMsg = document.createElement('p');
  emptyMsg.id = 'reseauEmpty';
  emptyMsg.style.cssText = 'display:none;text-align:center;color:var(--text-muted);font-family:var(--font-head);font-size:.9rem;padding:32px 0;width:100%;';
  emptyMsg.textContent = 'Aucun membre dans cette catégorie pour le moment.';
  ...
  function applyFilter(filter) {
    ...
    cards.forEach(card => {
      ...
      // Les cartes .scroll-reveal démarrent à opacity:0.
      // L'IntersectionObserver ne se redéclenche pas après un display:none,
      // donc on force la classe `revealed` dès qu'une carte est affichée.
      if (show) {
  ...
  // Appliquer le filtre initial (all) pour s'assurer que revealed est posé
  applyFilter('all');
})();
